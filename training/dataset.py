...
class Dataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        return_clear = False,   # For ablation study
    ):
        self._name = name
        ...
        self.return_clear = return_clear
    ...
    def set_mollifier(self, mollifier):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_clear:
                return image.copy(), self.get_label(idx), image_clear
...
class ImageFolderDataset(Dataset):
    def __init__(self,
        ...
        base_sigma      = 0.0,  # Base noise level.
        base_noise_seed            = 0, # Random seed for noise.
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        elif is_dir_seq(self._path):
            self._type = 'dir_seq'
            self._all_fnames = set()
            start, end = is_dir_seq(self._path)
            self._path = os.path.dirname(self._path)

            for i in range(int(start), int(end) + 1):                
                path_tmp = os.path.join(self._path, f"{i:05}")                
                self._all_fnames |= { os.path.join(f"{i:05}", os.path.relpath(os.path.join(root, fname), start=path_tmp)) for root, _dirs, files in os.walk(path_tmp) for fname in files}
    ...
    def _open_file(self, fname):
        if self._type == 'dir' or self._type == 'dir_seq':
